...
final class MainFrame implements ComponentListener, RefreshListener,
    ...
    MainFrame(JFrame frame) {
        ...
        if (!ApplicationSettings.CONSOLE_VIEW_ENABLED.getValue())
            this.setTabVisible(GUIMediator.CONSOLE_INDEX, false);
    ...
    private void buildTabs() {
        ...
        tabs.add(new SearchDownloadTab(SEARCH_MEDIATOR, DOWNLOAD_MEDIATOR));
        tabs.add(new MonitorUploadTab(MONITOR_VIEW, UPLOAD_MEDIATOR));
        tabs.add(new ConnectionsTab(CONNECTION_MEDIATOR));
        tabs.add(new LibraryPlayListTab(LIBRARY_MEDIATOR));
        
        if (CommonUtils.isLog4JAvailable()) {
            Console console = new Console();
            tabs.add(new ConsoleTab(console));
        }
        
        TABS = (Tab[])tabs.toArray(new Tab[0]);
    }
