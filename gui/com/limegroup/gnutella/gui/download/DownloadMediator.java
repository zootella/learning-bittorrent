...
public final class DownloadMediator extends AbstractTableMediator
    ...
    /**
     * Opens a file-chooser dialog and lets the user choose a torrent file
     * specifying a torrent to download.
     */
    public void openTorrent() {
    	File file = FileChooserHandler.getInputFile(GUIMediator.getAppFrame(),
    			TorrentFileFilter.INSTANCE);
    	if (file == null)
    		return;
    	try {
    		add(RouterService.downloadTorrent(file));
    	} catch (java.io.FileNotFoundException fnfe) {
    		ErrorService.error(fnfe);
    	} catch (IOException ioe) {ioe.printStackTrace();
    		// could not read torrent file or bad torrent file.
    		GUIMediator.showError("ERROR_COULD_NOT_OPEN_TORRENT", "\""
    				+ file.getName() + "\".",
    ...
    void launchSelectedDownloads() {
        ...
        for(int i = 0; i < lines.length; i++) {
            ...
            QUEUE.add(new Runnable() {
                public void run() {
                    if(dl.getState() == Downloader.INVALID) {
